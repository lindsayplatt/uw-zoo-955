...
# A: The distribution is close to normal, but has some extreme values on the high end.

# Q: Is the Spearman or Pearson correlation a better choice here and why?
# A: Spearman may be a slightly better choice here because absent intermediate values make it difficult to say the relationship is linear, 
# but it appears lose to linear so Pearson is also probably fine. 
...
# A: The QQ plots appear to show the same deviation from normality as the histograms. This is especially apparent when you plot a line
      qqline(mtcars$wt)
      qqline(mtcars$mgp)
...
# A: Yes, they appear to be tidy. Since there are not multiple observations of each car type, 
# we can think of the row names as an additional variable "car_type" 
...
# A: For every 1000 lb increase in weight, there is a decrease of 5mpg (slope = -5)

# Q: the intercept?
# A: about 35 mpg

# Q: the df for the error?  Why?
# A: 31 (number of observations minus 1 predictor variable)
...
# Residuals vs. fitted - looking for no obvious trend pattern in the residuals and equal variance across the range of fitted values. 
# We see a slight pattern where residuals are only positive at low and high fitted values, which suggests potential non-linearity.
# It's hard to say due to paucity of data at low and high ends.

# Normal Q-Q - lookjng for a linear relationship, we see slight deviation at the tails/extremes

# Scale-Location plot - similar to residuals versus fitted, looking for no pattern and equal variance. 
# This plot looks pretty good (but still not much data extremes).

# Residuals vs. leverage - looking for no influential points (i.e. points at the extreme that may be driving the relationship). 
# We see one potentially influential point that could be an outlier (Chrysler Imperial)
...
# A: Yes, the slope and intercept are very close to our estimates. We were 1 off on error df (30 rather than 31)
...
# A: The mean square for wt is much higher than for the residuals. 
# This suggests that much of the variance in mpg is explained by wt, with little variance leftover
# To obtain p-value we perform an F-test, dividing MSR (wt) by MSE (residuals) to calculate the test statistic 
# and comparing to the F distribution with appropriate degrees of freedom.
...
# A: If you didn't do this, it's not reproducible - if you change the model, the values are no longer relevant
...
# A: We are 95% confident that the true population mean lies between the upper and lower values. 
# In other words, 95% of random samples drawn from the same population would have a mean value within the confidence interval.

# Q: if you used interval="prediction" instead, what would be the interpretation? would the ribbon be wider or narrower?
# A: The prediction interval tells us the range within which the mpg of a new observation (i.e. car) at given weight would fall 95% of the time
# the ribbon would be wider because there is more uncertainty associated with predicting an individual value than the population mean
...
# A: These will all be the same because centering wt changes the absolute but not relative values of the data
...
# A: the units of a z-score are standard deviations from the mean. this is because all values are scaled to be relative to the mean of the data
# This makes it possible to compare effects of variables that operate on vastly different scales, but may makes intepretation more difficult
...
# ANOVA assumes normal distribution and equal vairance in the response, and that the data were collected independently. 
# We can roughly tell that the data have equal variance (i.e. no concentrated high or low values) and are normal (means are roughly centered).
# Independence is a property of the experimental design and cannot be checked visually
...
# Residual vs. fitted - checking for equal variance (homoscedacticity), looks good (no pattern in residuals)
# Normal Q-Q checking for normality, looks good (straight line)
# Scale location - checking for equal variance (homoscedacticity), looks good (no pattern)
# Constant leverage - checking for influential points, looks good (no pattern in with-level variation)
...
# A: The estimate tells you the mean value of the reference level (in this case setosa)
# the t value for the intercept tells whether the mean is different from zero. 
# the t values for the other levels tell whether the mean is different from the reference level
# R2 tells you how well the model fits the data (a metric of how much variance is explained by the model)
# F and its p-value tell whether the null hypothesis (in this case, that sepal width does not differ between species) is true 
...
# A: the species df is the number of levles (3) minus 1. the residual df is the number of observations minus the number of levels
# the species MS is much higher than the residuals, which suggests a substantial portion of the variation in iris sepal width can be explained its species
...
# A: all pairs of iris species have diffent mean sepal widths
...
# A: All of the coefficient estimates, errors, t and p values change because the reference category has changed (and thus comparison are different).
# the residuals, R2, F statistic and its p value do not change because they are based on the full model
...
# Residual vs. fitted - looks good, no pattern in residuals
# Normal Q-Q - looks okay, some extreme values on the high end
# Scale location - looks good, no pattern
# Residual vs. leverage - looks good, no influential points
...
# fourth, interpret results
# Q: interpret your results
# A: Both wt and hp are significant predictors of mpg. 
# mpg decreases with both wt and hp. together they explain ~82% of the variation in car mpg
...
# A: We predict that number of eggs laid varies with both limpet density and season - it decreases with density and is higher in the spring
# Q: use the picture to estimate the slope for each season. what are the units of the slope?
# A: For spring, we estimate the slope is about -0.05 eggs laid per fish (per volume? not sure what density units are)
#    For summer, we we estimate the slope is also about -0.05 eggs laid per fish per volume

# Q: do you expect the effect of density depends on season? that is, do you predict an interaction? explain why or why not
# A: We do not predict an interaction - the slope appears constant regardless of season.
...
# A: eggs laid = 2.664166 - 0.033650*denisty
...
# A: eggs laid = 1.851884 - 0.33650*density
